...
<div {{ $attributes->class($isHero ? 'booking-field relative min-w-0' : 'relative min-w-0 px-4 py-3') }}>
    ...
    >
        ...
        <div
            ...
            x-cloak
            x-transition
            class="airport-suggest absolute inset-x-0 top-full z-50 mt-2 overflow-hidden rounded-xl bg-white py-1 shadow-2xl ring-1 ring-slate-100"
        >
            <template x-for="(item, index) in suggestions" :key="item.code + '-' + index">
                ...
                >
                    <span class="mt-0.5 rounded-md bg-brand-700 px-1.5 py-0.5 text-[10px] font-bold tracking-wide text-white" x-text="item.code"></span>
                    <span class="min-w-0">
                        <span class="block truncate text-sm font-semibold text-slate-800" x-text="item.name"></span>
                        <span class="block truncate text-xs text-slate-500" x-text="item.city"></span>
                    </span>
                </button>
            </template>
        </div>
